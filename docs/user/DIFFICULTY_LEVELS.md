...
Используется **адаптивное снижение минимального расстояния**:
- Каждые 100 попыток проверяется прогресс
- Если точки не добавляются - расстояние снижается на 15%
- Максимум 3 снижения (до 45% от исходного расстояния)

| Уровень | Начальное minDist | После 3 снижений | Результат |
|---------|------------------|------------------|-----------|
| 🟢 Новичок | × 0.4 | × 0.22 | **100%** точек |
| 🟡 Любитель | × 0.8 | × 0.44 | **100%** точек |
| 🔴 Эксперт | × 1.2 | × 0.66 | **100%** точек |

### Как это работает

```
Попытка 1-100:   minDist = 300м (исходное)
Попытка 101-200: minDist = 255м (-15%)  ⚠️ Снижение шаг 1
Попытка 201-300: minDist = 210м (-30%)  ⚠️ Снижение шаг 2
Попытка 301+:    minDist = 165м (-45%)  ⚠️ Снижение шаг 3
```

**Примечание:** Адаптивное снижение применяется только если точки перестали добавляться. Если генерация идёт успешно - исходное расстояние сохраняется.
